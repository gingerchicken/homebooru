...
def register(request):
    if request.method == "POST":
        ...
        if not boorutils.is_valid_password(password):
            return render(request, 'booru/users/register.html', {'error': 'Password is not valid, make sure it: includes at least one digit, contains a special character and is longer than 6 characters.', 'username': username, 'email': email}, status=400)
